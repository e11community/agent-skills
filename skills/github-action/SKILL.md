---
name: github-action
description: Use when authoring, modernizing, or reviewing a TypeScript GitHub Action, OR when writing/reviewing any GitHub Actions workflow (`.github/workflows/*.yml`) — setting up action.yml, bundling src into a committed dist/ with esbuild, choosing the node runtime, pinning/swapping the actions a workflow uses (incl. our e11community forks), handling service-account/secret auth, running CLIs via @actions/exec, deprecating inputs, or forking an existing action.
---
...
**Writing or reviewing any GitHub Actions workflow** (`.github/workflows/*.yml`),
even in a repo that ships no action of its own — this skill's conventions for the
actions a workflow _consumes_ apply just as much to a plain CI/CD workflow:

- Which ref to pin an action to ([Pinning actions](#pinning-actions): vendor →
  current major tag; community → SHA + version comment)
- Which upstream actions to swap for our e11community forks
  ([Swap these out](#swap-these-out-for-our-e11community-forks))
- Wiring Dependabot to keep those refs current with commitlint-clean messages
- The reusable `validate.yml` / `pr-title.yml` / `release.yml` templates

Not for: composite (`using: composite`) or Docker (`using: docker`) actions —
those don't bundle JS.
...
### Swap these out for our e11community forks

Before you pin _anything_, check this list first. We maintain first-party forks of
a few community actions; when a workflow (or a fork you're modernizing) references
the upstream on the left, replace it with our fork on the right. These are our own
org, so treat them like platform vendors — ride the **current major tag**, no SHA
pin. Because you're replacing the upstream outright, **delete any recommended
version or SHA pin the upstream had** in the tables below.

| Swap out (upstream)                    | Use instead (e11community)    |
| -------------------------------------- | ----------------------------- |
| `actions-ecosystem/action-regex-match` | `e11community/regex-match`    |
| `w9jds/setup-firebase`                 | `e11community/setup-firebase` |
...
| GitHub          | `actions/configure-pages`               | `@v6` |
| GitHub          | `actions/dependency-review-action`      | `@v5` |
| GitHub.         | `actions/deploy-pages`                  | `@v5` |
